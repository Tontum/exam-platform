<!--
  ProjectTools.vue — 学员端：项目工具页
  用户点击某个项目后进入此页面，展示该项目下管理员配置的工具列表
  当前阶段只实现试卷工具，后续可扩展作业、视频等工具
  设计风格：现代简约 + 清新蓝白配
-->
<template>
  <div class="project-tools-page">
    <!-- 返回按钮 -->
    <div class="back-button" @click="$router.push('/teacher/projects')">
...
onMounted(async () => {
  ...
  // 调用 API 获取已启用的工具列表
  try {
    const enabledTools = await listProjectTools(projectId)
    
    // 所有工具定义
    const allTools = [
      { code: 'paper', name: '试卷工具', description: '在线考试与测评' },
      { code: 'homework', name: '作业工具', description: '布置与提交作业' },
      { code: 'video', name: '视频工具', description: '视频学习资源' },
      { code: 'article', name: '文章工具', description: '阅读学习资料' },
      { code: 'chat', name: '交流工具', description: '在线讨论交流' },
      { code: 'qa', name: '问答工具', description: '提问与解答' },
      { code: 'live', name: '直播工具', description: '在线直播授课' },
    ]
...
function enterTool(tool: ToolVO) {
  ...
  switch (tool.code) {
    case 'paper':
      router.push(`/teacher/papers/${projectId}`)
      break
    case 'homework':
      // TODO: 跳转到作业列表
